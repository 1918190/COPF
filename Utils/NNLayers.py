...
def stopGradientOrNot(ind, inp, scale=0.1):
	ini_inp = inp * scale
	if ind == args.behNum - 1:
		ini_inp = tf.stop_gradient(ini_inp)
		index_ulat, index_ilat = tf.split(ini_inp, [args.user, args.item], 0)
	else:
		index_ulat, index_ilat = tf.split(ini_inp, [args.user, args.item], 0)
	return index_ulat, index_ilat
...
def FCthree(inp, outDim, name=None, useBias=False, activation=None, reg=False, useBN=False, dropout=None, initializer='xavier', reuse=False, biasReg=False, biasInitializer='zeros'):
	global params
	global regParams
	global leaky
	inDim = inp.get_shape()[2]
	temName = name if name!=None else 'defaultParamName%d'%getParamId()
	W = getOrDefineParam(temName, [inDim, outDim], reg=reg, initializer=initializer, reuse=reuse)
	if dropout != None:
		ret = tf.nn.dropout(inp, rate=dropout) @ W
...
def l2_normalize(inputs, axis=-1, epsilon=1e-12):
	"""Perform L2 normalization on the input tensor along the specified axis."""
	# Calculate the L2 norm along the specified axis
	square_sum = tf.reduce_sum(tf.square(inputs), axis, keepdims=True)
	# Prevent division by zero
	# norm = tf.sqrt(tf.maximum(square_sum, epsilon))
	norm = tf.sqrt(square_sum)
	# Normalize the input by the L2 norm
